...
local function build()
    ...
    card.killLabel = label("Убийств в ЦЛК 25")
...
local function hideBody()
    ...
    for i = 1, 3 do hideFs(card.kills[i]); hideFs(card.achs[i]) end
...
local function renderSub(shown, class, s, y, id, who)
    ...
    if who then
        local bits = {}
        if who.online == false then tinsert(bits, "не в сети") end
...
local function renderNotInBase(name, y, whoState)
    ...
    if live then
        if live.any then
            ...
            card.why:SetText("Статистика из игры. Логи в базе — с конца 2023 года.")
        else
            card.msg:SetText("Не ходил в ЦЛК 25: по статистике из игры ни одного кила")
...
local function renderMissing(rec, name, missing, y, whoState)
    ...
    card.msg:SetText("В сезоне " .. tostring(ns.CurrentSeason() or "?") .. " рейдов не было")
    y = placeLine(card.msg, y, 0, 0)
...
local function render(info)
    ...
    local who, whoState
    if not rec and not info.guid and not ns.DataProblem() and not (ns.UnitFor and ns.UnitFor(shown)) and ns.WhoInfo then
