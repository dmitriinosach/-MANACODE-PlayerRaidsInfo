...
local HOW = {
    ...
    "Закрывали — в какой доле тех рейдов босс убит. Если игрок пришёл позже (есть кил следующего босса, а этого нет), рейд для этого босса не считается. Танк весит x3, хил x2, ДД x1.",
...
local function experience(rec, mode, bi)
    ...
    local total, tried, killed, clean = #picked, 0, 0, 0
    for _, e in ipairs(picked) do
        local cells = e.raid.cells
        if cells[bi] then
            tried, killed = tried + 1, killed + 1
        else
            local later = false
            for j = bi + 1, 3 do
                if cells[j] then later = true end
            end
            if not later then tried = tried + 1 end
        end
        if e.raid.wipes == 0 then clean = clean + 1 end
    end
    if total == 0 then return nil end
    return tried > 0 and (killed / tried) or nil, clean / total
end
local function compute()
    local r = { exp = 0, worst = 0, hps = 0, noData = 0, t = 0, h = 0, d = 0, kw = 0, kww = 0, cw = 0, ww = 0 }
    local bi = ns.IsRS(pick.mode) and 1 or pick.boss
    for _, m in ipairs(roster) do
        ...
        if c then
            ...
            if k then r.kw, r.kww = r.kw + k * w, r.kww + w end
        end
        if st.avg then
            if st.role == "h" then
                r.hps = r.hps + st.avg
...
local function renderSummary()
    ...
    if r.ww > 0 then
        L[3]:SetText("Закрывали " .. (BOSS_GEN[boss] or ns.BOSS[boss]) .. ": " .. chanceText(r.kww > 0 and (r.kw / r.kww * 100) or 0)
...
local ROLE_NAME = { d = "ДД", h = "хил", t = "танк" }
local function rowTip(self)
    local st = self.stat
    if not st then return end
    local unit = st.role == "h" and " хпс" or " дпс"
    local rows = { { "Роль", ROLE_NAME[st.role] or "ДД" } }
    local src = st.avg and st or self.alt
    if src and src.avg then
        tinsert(rows, { "Среднее", ns.Compact(src.avg) .. unit })
        tinsert(rows, { "Минимум", ns.Compact(src.min) .. unit })
        tinsert(rows, { "Рейдов в расчёте", src.n or 0 })
        if src.ilvl then
            local low = src.ilvlLow or src.ilvl
            tinsert(rows, { "ilvl рейдов", low == src.ilvl and tostring(src.ilvl) or (low .. "-" .. src.ilvl) })
        end
    end
    local foot
    if not st.avg and self.alt then
        foot = "В этой сложности рейдов нет, показано " .. (self.altLabel or "")
    elseif not st.avg then
        foot = "Рейдов на этом боссе нет — в сумму идёт как 0"
    elseif st.season then
        foot = "Последние рейды — из сезона " .. st.season
    end
    ns.TipTable(self, "ANCHOR_RIGHT", self.tipName or "", rows, foot)
end
