...
function ns.MissingText()
    local long = ns.Meta().complete == false and "Выгрузка неполная: мог ещё не попасть в неё." or nil
    return "Не ходил в рейды или ходил до начала сбора рейд-логов (2023)", long
end
...
local function parseSeason(block)
    local sn, gs, last, spec, raids, bd, bh, list, kills, roles, bt, rspec = strsplit(";", block)
    local s = {
...
function ns.HistLine(rec)
    ...
    for _, group in ipairs({ { "lich", { "ih", "iu", "in" } }, { "hal", { "rh", "rn" } } }) do
        ...
        if bestMode then
            tinsert(bits, ns.BOSS[boss] .. " " .. HIST_SHORT[bestMode] .. " " .. ns.Color("white", "x" .. bestN))
        end
    end
...
ns.SEASON_CIRCLE = {
    { sn = 3, circle = 7, from = "01.04.2026", dates = "с 01.04.2026" },
    { sn = 2, circle = 6, from = "12.08.2025", dates = "12.08.2025 - 12.12.2025" },
    { sn = 1, circle = 5, from = "01.11.2024", dates = "01.11.2024 - 31.03.2025" },
    { sn = 0, circle = 4, from = "20.12.2023", dates = "20.12.2023 - 18.06.2024" },
}
ns.ALL_MODES_FROM = 3
local EARLY_MODES = { ih = true, iu = true }
function ns.ModeCollected(sn, mode)
    if type(sn) ~= "number" or sn >= ns.ALL_MODES_FROM then return true end
    return EARLY_MODES[mode] or false
end
...
function ns.SeasonMessage(season, why, reason)
    ...
    if why == "nofile" then
        return "Сезон " .. season .. " не скачан — запустите ОбновитьДанные.exe или распакуйте season" .. season .. ".zip"
    end
    if why == "empty" then return ns.SeasonEmptyText(season) end
    return nil
end
