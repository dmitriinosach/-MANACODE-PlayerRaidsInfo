...
local function specKey(spec)
    ...
    return SPEC_BY_RU[key] or key
end
function ns.LearnSpecIcon(classFile, tabName, icon)
    if not classFile or type(tabName) ~= "string" or type(icon) ~= "string" or icon == "" then return end
    PlayerRaidsDB.specIcons = PlayerRaidsDB.specIcons or {}
    local box = PlayerRaidsDB.specIcons[classFile] or {}
    box[specKey(tabName)] = icon
    PlayerRaidsDB.specIcons[classFile] = box
end
function ns.SpecIcon(classFile, spec)
    if not classFile or not spec or spec == "" then return nil end
    local learned = PlayerRaidsDB.specIcons and PlayerRaidsDB.specIcons[classFile]
    local hit = learned and learned[specKey(spec)]
    if not hit and learned and specKey(spec) == "feral" then hit = learned.feralcombat end
    local spells = SPEC_SPELL[classFile]
    if not spells then return hit end
    local key = specNorm(spec)
    local id = spells[key] or spells[SPEC_BY_RU[key] or ""]
    if not id or type(GetSpellInfo) ~= "function" then return hit end
...
ns.MODE_COUNT = { ih = "ЦЛК гер", iu = "ЦЛК анбаф", ["in"] = "ЦЛК об", rh = "РС гер", rn = "РС об" }
...
ns.REPO_URL = "https://github.com/dmitriinosach/-MANACODE-PlayerRaidsInfo"
ns.DISCORD_URL = "https://discord.gg/hpTkJCJbwn"
ns.RELEASES_URL = "https://github.com/dmitriinosach/-MANACODE-PlayerRaidsInfo/releases"
ns.RELEASE_URL = "https://github.com/dmitriinosach/-MANACODE-PlayerRaidsInfo/releases/latest"
ns.DATA_URL = "https://github.com/dmitriinosach/-MANACODE-PlayerRaidsInfo/tree/data"
ns.BOBER_URL = "https://boberlogs.top"
...
ns.ART = "Interface\\AddOns\\" .. ADDON .. "\\bin\\art\\"
...
ns.BTN = {
    ...
    bg = { 0, 0, 0, 0.7 }, border = { 1, 0.82, 0, 0.35 }, text = { 0.8, 0.8, 0.8 },
    bgHover = { 0, 0, 0, 0.45 }, borderHover = { 1, 0.82, 0, 1 }, textHover = { 1, 1, 1 },
    bgDown = { 0, 0, 0, 0.85 },
    bgOn = { 0, 0, 0, 1 }, bgOnHover = { 0, 0, 0, 1 }, borderOn = { 1, 0.93, 0.55, 1 },
    fillOn = { 1, 0.82, 0, 1 }, fillOnHover = { 1, 0.9, 0.3, 1 },
    textOn = { 0, 0, 0 },
...
function ns.PaintButton(b)
    ...
    if b.fill then
        if b.on and not b.off then
            local f = b.hovered and c.fillOnHover or c.fillOn
            b.fill:SetVertexColor(f[1], f[2], f[3], f[4])
            b.fill:Show()
        else
            b.fill:Hide()
        end
...
function ns.ShowCopy(anchor, text, width, up)
    ...
    copy:ClearAllPoints()
    if up then
        copy:SetPoint("BOTTOMLEFT", anchor, "TOPLEFT", -8, 3)
    else
        copy:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", -8, -3)
    end
    copy:Show()
    copy.hint:SetText("Выделено, нажмите Ctrl+C")
    copy.value = text
