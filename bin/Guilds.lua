...
function ns.GearScoreText(id, name)
    local v, age = ns.GearScoreOf(id)
    if not v and not id and name then
        local day
        v, day = gsFromAddon(name)
        age = day and (today() - day) or nil
    end
...
local WHO_WAIT = 5
local WHO_GAP = 6
local WHO_FRESH = 1800
local WHO_MISS = 60
local who = {}
local whoPending, whoAsked, whoLast = nil, 0, -100
local whoWatch = CreateFrame("Frame")
whoWatch:Hide()
local function whoRelease()
    whoPending = nil
    whoWatch:UnregisterEvent("WHO_LIST_UPDATE")
    whoWatch.restore = true
    whoWatch:Show()
end
local function whoRead(name)
    local low = ns.Lower(name)
    for i = 1, (GetNumWhoResults() or 0) do
        local n, guild, level, race, class, zone, classFile = GetWhoInfo(i)
        if n and ns.Lower(n) == low then
            return { guild = guild ~= "" and guild or false, level = level, class = classFile, zone = zone }
        end
    end
    return { miss = true }
end
whoWatch:SetScript("OnEvent", function()
    if not whoPending then return end
    local name = whoPending
    local out = whoRead(name)
    out.t = GetTime()
    who[name] = out
    whoRelease()
    if ns.Refresh then ns.Refresh(name) end
end)
whoWatch:SetScript("OnUpdate", function(self)
    ...
    if whoPending and GetTime() - whoAsked > WHO_WAIT then whoRelease() end
end)
function ns.WhoInfo(name)
    if not name or name == "" then return nil end
    local hit = who[name]
    if hit and GetTime() - hit.t < (hit.miss and WHO_MISS or WHO_FRESH) then
        if hit.miss then return nil end
        return hit
    end
    if type(SendWho) ~= "function" or type(SetWhoToUI) ~= "function" or type(GetWhoInfo) ~= "function" then return nil end
    if whoPending or whoWatch.restore then return nil, whoPending == name and "wait" or nil end
    if GetTime() - whoLast < WHO_GAP then return nil end
    if FriendsFrame and FriendsFrame:IsShown() then return nil end
    whoPending, whoAsked, whoLast = name, GetTime(), GetTime()
    if FriendsFrame and FriendsFrame.UnregisterEvent then FriendsFrame:UnregisterEvent("WHO_LIST_UPDATE") end
    whoWatch:RegisterEvent("WHO_LIST_UPDATE")
    whoWatch:Show()
    SetWhoToUI(1)
    SendWho("n-" .. name)
    return nil, "wait"
...
local CHAT_EVENTS = {
    "CHAT_MSG_CHANNEL", "CHAT_MSG_SAY", "CHAT_MSG_YELL", "CHAT_MSG_WHISPER", "CHAT_MSG_GUILD",
    ...
    "CHAT_MSG_BATTLEGROUND_LEADER", "CHAT_MSG_EMOTE",
}
local chatGuid = {}
local chatWatch = CreateFrame("Frame")
for _, e in ipairs(CHAT_EVENTS) do chatWatch:RegisterEvent(e) end
chatWatch:SetScript("OnEvent", function(self, event, ...)
    ...
    if type(guid) ~= "string" or not string.match(guid, "^0x%x+$") then return end
    sender = string.match(sender, "^([^%-]+)") or sender
    chatGuid[ns.Lower(sender)] = guid
end)
function ns.ChatGuid(name)
    return name and chatGuid[ns.Lower(name)] or nil
end
