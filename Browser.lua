...
local function buildHeader(keep)
    ...
    right.spec:SetScript("OnEnter", function(self)
        if self.name then ns.Tip(self, "ANCHOR_TOP", self.name) end
    end)
...
local function buildOptions()
    ...
    y = y + 24
    local sep2 = line(optPanel, 0.2)
    sep2:SetPoint("TOPLEFT", optPanel, "TOPLEFT", 10, -y)
    sep2:SetPoint("TOPRIGHT", optPanel, "TOPRIGHT", -10, -y)
    local about = ns.Text(optPanel, 13)
    gold(about)
    about:SetPoint("TOPLEFT", 12, -(y + 8))
    about:SetText("Об аддоне")
    local ver = ns.Text(optPanel, 12)
    dim(ver)
    ver:SetPoint("LEFT", about, "RIGHT", 8, 0)
    ver:SetText("версия " .. (ns.LinkVersion and ns.LinkVersion() or "?"))
    y = y + 30
    local links = {
        { "GitHub", ns.REPO_URL, "Страница аддона: релизы, описание, данные" },
        { "Discord Manacode", ns.DISCORD_URL, "Вопросы, новости и новые версии" },
    }
    local x = 12
    for _, it in ipairs(links) do
        local b = ns.MakeButton(optPanel, 12, nil, 20)
        ns.FitButton(b, it[1], 18)
        b:SetPoint("TOPLEFT", optPanel, "TOPLEFT", x, -y)
        b.onClick = function(self) ns.ShowCopy(self, it[2], 300) end
        b.tip = function(self) ns.Tip(self, "ANCHOR_TOP", it[1], it[3]) end
        x = x + b:GetWidth() + 6
    end
    optPanel:SetHeight(y + 34)
end
...
local function buildStatus()
    ...
    gold(newText)
    newText:SetPoint("LEFT", status, "RIGHT", 14, 0)
    newText:Hide()
    ...
    get.tip = function(self)
        ns.Tip(self, "ANCHOR_TOP", "Новая версия аддона", "Покажет ссылку на страницу релиза — скопируйте её в браузер.")
    end
    get:Hide()
    ...
    if ns.OnNewVersion then ns.OnNewVersion(showNew) end
    showNew()
end
